...
def load_state(path, model, optimizer=None, use_pavi=False):
    # def map_func(storage, location):
    #    return storage.cuda()
    if use_pavi:
        print("=> loading checkpoint from PAVI")
        checkpoint = pavi.get_snapshot(path)
        if not checkpoint:
            print("=> no checkpoint found on PAVI")
            return
    else:
